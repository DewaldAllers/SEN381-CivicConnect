...
The Part 2 branch is `codex/m1-part2-requirements-foundation`. Record its review PR when available. There are no active GitHub issues for these requirements; the earlier setup issues were deleted at the user's request. If issues are created later through the agreed team workflow, add their links alongside the requirement and PR. Issue history has not been reconstructed.
