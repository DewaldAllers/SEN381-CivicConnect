...
Working notes for the individual engineering defence. These are prompts, not a script to read out. SRC-M1 section 6.2.1 (p. 6) assesses delivery on explaining, not reading, and section 6.2.2 (p. 6) assesses command of the artefacts themselves. Open the live files during questioning instead of describing them from memory.
...
This answers several questions at once. It shows the register is live and maintained, it shows a risk connecting to a real issue as SRC-MASTER section 12 (p. 12) requires, and it shows the team recording an uncomfortable fact instead of hiding it.
...
Pick this thread and follow it live instead of describing it.
...
The risk probabilities are judgements without historical data. DEC-005 records why the team used a 3 by 3 scale instead of a 5 by 5, which is honest about the precision available.

Branch protection was only applied on 9 September. Everything merged before that, including PR #20, entered `main` with nothing enforcing the two-approval rule.
...
RSK-006 names the Protection of Personal Information Act as the obligation most likely to apply, and the team has not confirmed that. It is recorded as missing information, not as an established legal position.
